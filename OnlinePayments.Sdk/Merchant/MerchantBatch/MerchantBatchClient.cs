...
namespace OnlinePayments.Sdk.Merchant.MerchantBatch
{
    ...
    /// </summary>
    public class MerchantBatchClient : ApiResource, IMerchantBatchClient
    {
        public MerchantBatchClient(ApiResource parent, IDictionary<string, string> pathContext) :
        ...
        public async Task<SubmitBatchResponse> SubmitBatch(SubmitBatchRequestBody body, CallContext context = null)
        {
            var uri = InstantiateUri("/v2/{merchantId}/merchant-batches", null);
            try
            {
                context = context ?? new CallContext();
                context.GZip = true;

                return await _communicator.Post<SubmitBatchResponse>(
                        uri,
                        ClientHeaders,
                        null,
                        body,
        ...
        ///            or the service that you're trying to reach is temporary unavailable (HTTP status code 500, 502 or 503)</exception>
        /// <exception cref="ApiException">if the payment platform returned any other error</exception>
        public async Task ProcessBatch(string merchantBatchReference, CallContext context = null)
        {
            var pathContext = new Dictionary<string, string>
            {
                { "merchantBatchReference", merchantBatchReference }
            };
            var uri = InstantiateUri("/v2/{merchantId}/merchant-batches/{merchantBatchReference}/process", pathContext);
            try
            {
        ...
        ///            or the service that you're trying to reach is temporary unavailable (HTTP status code 500, 502 or 503)</exception>
        /// <exception cref="ApiException">if the payment platform returned any other error</exception>
        public async Task<GetBatchStatusResponse> GetBatchStatus(string merchantBatchReference, CallContext context = null)
        {
            var pathContext = new Dictionary<string, string>
            {
                { "merchantBatchReference", merchantBatchReference }
            };
            var uri = InstantiateUri("/v2/{merchantId}/merchant-batches/{merchantBatchReference}", pathContext);
            try
            {

                return await _communicator.Get<GetBatchStatusResponse>(
                        uri,
                        ClientHeaders,
                        null,
                        context).ConfigureAwait(false);
        ...
        /// </summary>
        /// <param name="merchantBatchReference">string</param>
        /// <param name="query">GetPaymentsReportParams</param>
        /// <param name="context">CallContext</param>
        /// <returns>PaymentsReportResponse</returns>
        /// <exception cref="IdempotenceException">if an idempotent request caused a conflict (HTTP status code 409)</exception>
        /// <exception cref="ValidationException">if the request was not correct and couldn't be processed (HTTP status code 400)</exception>
        /// <exception cref="AuthorizationException">if the request was not allowed (HTTP status code 403)</exception>
        /// <exception cref="ReferenceException">if an object was attempted to be referenced that doesn't exist or has been removed,
        ...
        ///            or the service that you're trying to reach is temporary unavailable (HTTP status code 500, 502 or 503)</exception>
        /// <exception cref="ApiException">if the payment platform returned any other error</exception>
        public async Task<PaymentsReportResponse> GetPaymentsReport(string merchantBatchReference, GetPaymentsReportParams query, CallContext context = null)
        {
            var pathContext = new Dictionary<string, string>
            {
                { "merchantBatchReference", merchantBatchReference }
            };
            var uri = InstantiateUri("/v2/{merchantId}/merchant-batches/{merchantBatchReference}/reports/payments", pathContext);
            try
            {

                return await _communicator.Get<PaymentsReportResponse>(
                        uri,
                        ClientHeaders,
                        query,
                        context).ConfigureAwait(false);
